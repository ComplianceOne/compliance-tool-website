...
// Reveal Animation
export const revealAnimation = (scope?: HTMLElement | Document) => {
  const elements = (scope || document).querySelectorAll<HTMLElement>(
    '.it_text_reveal_anim'
  );
  ...
  elements.forEach((element) => {
    ...
    const onScroll = parseInt(
      element.getAttribute('data-on-scroll') || '1',
      10
    );
    const stagger = parseFloat(element.getAttribute('data-stagger') || '0.02');
    const delay = parseFloat(element.getAttribute('data-delay') || '0.05');
    ...
    if (onScroll === 1) {
      ...
      // Animation without ScrollTrigger
...
// Char Animation
export const charAnimation = (scope?: HTMLElement | Document) => {
  const elements = (scope || document).querySelectorAll<HTMLElement>(
    '.it-char-animation'
  );
